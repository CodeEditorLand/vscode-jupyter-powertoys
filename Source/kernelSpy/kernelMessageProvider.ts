...
} from "../kernelManager/vscodeJupyter";
import {
	IAnyMessageArgs,
	IKernelConnection,
...
class MessageTreeItem extends TreeItem {
	constructor(public readonly data: MessageNode) {
		...
		this.contextValue = `kernelMessageItem:${data.__type}:${
			data.clipboardText ? "canCopyToClipboard" : ""
		}`;
		if (data.__type === "parentMessageNode" || data.direction === "send") {
			...
			if (
				...
				if (
					...
					debugRequest.content.arguments &&
					typeof debugRequest.content.arguments === "object" &&
					typeof debugRequest.content.arguments["source"] ===
						"object" &&
					typeof debugRequest.content.arguments["source"]["path"] ===
						"string" &&
					...
					this.tooltip = `${
						debugRequest.content.arguments.source.path
					}\nlines: ${lines.join(", ")}`;
				}
			}
...
export class ActiveKernelMessageProvider
	implements IActiveKernelChildNodesProvider
{
	...
	public activate() {
		...
		this.disposables.push(
			...[
				...
				commands.registerCommand(
					"jupyter-kernelManager.kernelMessageCopy",
					(data: DataNode | MessageNode) => {
			...
			if (Array.isArray(data)) {
				return data.map((value, index) => {
					...
					return <DataNode>{
						...
						msg: ourNode.msg,
						hasChildren:
							hasChildren && !isEmptyObject && !isEmptyArray,
						clipboardText: getTextForClipboard(value),
					};
				});
	...
	getTreeItem(node: ICustomNodeFromAnotherProvider): TreeItem {
		...
		if (
			...
			(ourNode.__type === "messageNode" ||
				ourNode.__type === "parentMessageNode")
		) {
	...
	private addHandler(connection: IKernelConnectionInfo, parent?: RootNode) {
		...
		this.disposables.push(
			new Disposable(() =>
				connection.connection.anyMessage.disconnect(anyHandler)
			)
		);
		...
		this.disposables.push(
			new Disposable(() =>
				connection.connection.iopubMessage.disconnect(ioPubHandler)
			)
		);

		const unhandledHandler = this.onUnhandledMessageHandler.bind(
			this,
			parent
		);
		...
		this.disposables.push(
			new Disposable(() =>
				connection.connection.unhandledMessage.disconnect(
					unhandledHandler
				)
			)
		);
	...
	private onAnyMessageHandler(
		...
		if (info) {
			...
			if (
				...
				requestsById.set(args.msg.header.msg_id, {
					parent: message,
					children: [{ ...message }],
				});
