...
export function getCellRunGroupMetadata(cell: vscode.NotebookCell): string {
	const customMetadata = useCustomMetadata()
		? cell.metadata?.custom?.metadata?.notebookRunGroups
		: cell.metadata?.metadata?.notebookRunGroups;
...
export function updateCellRunGroupMetadata(
	...
	if (useCustomMetadata()) {
		...
		newMetadata.custom.metadata.notebookRunGroups.groupValue =
			newGroupValue;
	} else {
