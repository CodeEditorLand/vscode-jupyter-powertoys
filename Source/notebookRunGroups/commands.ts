...
import {
	getCellRunGroupMetadata,
	updateCellRunGroupMetadata,
} from "./util/cellMetadataHelpers";
...
export function registerCommands(context: vscode.ExtensionContext) {
	...
	context.subscriptions.push(
		vscode.commands.registerCommand(
			"vscode-notebook-groups.addGroup1",
			(args) => {
	...
	context.subscriptions.push(
		vscode.commands.registerCommand(
			"vscode-notebook-groups.removeGroup1",
			(args) => {
	...
	context.subscriptions.push(
		vscode.commands.registerCommand(
			"vscode-notebook-groups.executeGroup1",
			(args) => {
...
function getCurrentActiveCell(): vscode.NotebookCell | undefined {
	...
	if (activeNotebook) {
		...
		return activeNotebook.notebook.cellCount >= 1
			? activeNotebook.notebook.cellAt(selectedCellIndex)
			: undefined;
	}
}

function removeGroupFromCustomMetadata(
	notebookCell: vscode.NotebookCell,
	targetRunGroup: RunGroup,
) {
	...
	log(
		`Removing from group Cell Index: ${notebookCell.index} Groups Value: ${targetRunGroup.toString()}`,
	);
}

function addGroupToCustomMetadata(
	notebookCell: vscode.NotebookCell,
	targetRunGroup: RunGroup,
) {
	const currentValue = getCellRunGroupMetadata(notebookCell);
	...
	log(
		`Adding to group Cell Index: ${notebookCell.index} Groups Value: ${newValue}`,
	);
}
