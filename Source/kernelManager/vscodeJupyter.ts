...
export async function getEnvironmentVersionFromUri(
	uri: Uri | undefined,
	api: PythonExtension,
) {
	if (!uri) {
...
export function getEnvironmentType({
	...
	for (const tool of tools) {
		...
		if (
			...
			return KnownEnvironmentToolsToEnvironmentTypeMapping.get(
				tool as unknown as KnownEnvironmentTools,
			)!;
		}
	}
...
export async function getPythonEnvironmentName(
	uri: Uri | undefined,
	api: PythonExtension,
) {
	if (!uri) {
