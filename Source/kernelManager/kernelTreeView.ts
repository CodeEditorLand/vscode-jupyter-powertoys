...
export async function getDisplayNameOrNameOfKernelConnection(
	...
	switch (kernelConnection.kind) {
		...
		case "startUsingLocalKernelSpec": {
			...
			const envNamePromise = getPythonEnvironmentName(
				kernelConnection.interpreter?.uri,
				pythonApi,
			);
...
class KernelSpecTreeItem extends TreeItem {
	constructor(
		public readonly data: IKernelSpecTreeNode,
	...
	async getTreeItem(element: Node): Promise<TreeItem> {
		switch (element.type) {
			...
			case "activeRemoteKernel": {
				const item = new ActiveLocalOrRemoteKernelConnectionTreeItem(
					element,
					this.pythonApi,
				);
				await item.resolve();
				...
				specs.map(async (k) => {
					try {
						return {
							...k,
							displayName:
								await getDisplayNameOrNameOfKernelConnection(
									k,
									this.pythonApi,
								),
						};
