...
import {
	ActiveKernelChildNodesProviderRegistry,
	IActiveKernelChildNodesProvider,
} from "./kernelChildNodeProvider";
	...
	switch (kernelConnection.kind) {
		...
		case "startUsingPythonInterpreter":
			if (
				...
				if (
					...
					const envName =
						...
						(kernelConnection.interpreter.envPath
							? path.basename(
									kernelConnection.interpreter.envPath.fsPath
							  )
							: "");
					if (envName) {
				...
				return kernelConnection.interpreter.envName
					? `${kernelConnection.interpreter.envName} ${
							pythonDisplayName ? `(${pythonDisplayName})` : ""
					  }`
					: pythonDisplayName;
			}
	}
	...
	constructor(public readonly data: IKernelSpecTreeNode) {
		super(
			getDisplayNameOrNameOfKernelConnection(
				data.kernelConnectionMetadata
			),
			TreeItemCollapsibleState.None
		);
		switch (data.kernelConnectionMetadata.kind) {
			case "startUsingLocalKernelSpec":
				this.description = data.kernelConnectionMetadata.kernelSpec
					.specFile
					? getDisplayPath(
							data.kernelConnectionMetadata.kernelSpec.specFile
					  )
					: "";
				break;
			case "startUsingPythonInterpreter":
				this.description = getDisplayPath(
					data.kernelConnectionMetadata.interpreter.uri.fsPath
				);
				break;
			default:
				break;
...
class ActiveLocalOrRemoteKernelConnectionTreeItem extends TreeItem {
	constructor(
		...
		if (
			data.uri &&
			!data.uri.fsPath.endsWith(iPyNbNameToTemporarilyStartKernel)
		) {
		...
		this.contextValue = `${prefix}:activeKernel:${
			this.data.connection?.connection.status || "dead"
		}`;
		...
		if (
			...
			if (
				tooltips.length === 0 &&
				data.kernelConnectionMetadata.kernelModel.execution_state
			) {
				tooltips.push(
			...
			if (time) {
				tooltips.push(
					`Last activity ${new Date(time).toLocaleString()}`
				);
			}
			// const connections =
		...
		if (this.data.connection?.connection.connectionStatus) {
			tooltips.push(
				`Connection ${this.data.connection?.connection.connectionStatus}`
			);
		}
		...
		if (this.data.connection) {
			if (
				this.data.connection.connection.connectionStatus !== "connected"
			) {
				this.updateIcon(
					this.data.connection.connection.connectionStatus
				);
			} else {
...
export class KernelTreeView implements TreeDataProvider<Node> {
	...
	public async getChildren(element?: Node): Promise<Node[]> {
		if (!element) {
			this.cachedKernels = (
				await this.kernelService.getKernelSpecifications()
			).map((k) => {
				try {
					return {
						...k,
		...
		switch (element.type) {
			...
			case "activeKernelRoot": {
				...
				if (element.baseUrl) {
					...
					const remoteActiveKernelStartedUsingConnectToRemoveKernelSpec =
						remoteActiveKernels.filter(
							(item) =>
								item.metadata.kind ===
					...
					await Promise.all(
						this.cachedKernels
							...
							.map(
								(item) =>
									item as LiveRemoteKernelConnectionMetadata
							)
							...
							.map(async (item) => {
								...
								if (
									remoteActiveKernelStartedUsingConnectToRemoveKernelSpec.some(
										(activeRemote) => {
											const kernel =
												activeRemote.uri &&
											...
											return (
												kernel?.connection.connection
													.id === item.kernelModel.id
											);
										}
									)
								) {
								...
								if (item.kernelModel.id) {
									if (
										item.kernelModel.id &&
										uniqueKernelIds.has(item.kernelModel.id)
									) {
										return;
									}
								...
								const activeInfoIndex =
									remoteActiveKernels.findIndex(
										(activeKernel) =>
											activeKernel.metadata === item
									);
								const activeInfo =
									activeInfoIndex >= 0
										? remoteActiveKernels[activeInfoIndex]
										: undefined;
								if (activeInfoIndex >= 0) {
									remoteActiveKernels.splice(
										activeInfoIndex,
										1
									);
								}
					...
					remoteActiveKernels.forEach((item) => {
						...
						if (
							...
							if (
								kernel &&
								uniqueKernelIds.has(
									kernel.connection.connection.id
								)
							) {
								return;
							}
					...
					const localActiveKernelSpecs = activeKernels.filter(
						(item) => isLocalKernelConnection(item.metadata)
					);
					const localActiveKernelsWithInfo = await Promise.all(
						localActiveKernelSpecs
							...
							.map(async (item) => {
								const info = await this.kernelService.getKernel(
									item.uri!
								);
					...
					activeLocalKernelNodes.forEach((item) =>
						this.trackKernelConnection(item)
					);
			...
			case "customNodeFromAnotherProvider": {
				...
				if (provider) {
					...
					console.error(
						"Unknown provider for custom nodes",
						element.providerId
					);
					return [];
				}
	...
	public static register(
		...
		const treeView = window.createTreeView<Node>(
			"jupyterKernelsView",
			options
		);
	...
	private trackKernelConnection(
			...
			this.mappedActiveLocalKernelConnections.find(
				(item) =>
					...
					item.kernelConnectionMetadata ===
						localActiveKernel.kernelConnectionMetadata &&
					(item.uri
						? item.uri?.toString() ===
						  localActiveKernel.uri?.toString()
						: true) &&
		...
		localActiveKernel.connection.connection.connectionStatusChanged.connect(
			onConnectionStatusChanged,
			this
		);
		localActiveKernel.connection.connection.statusChanged.connect(
			onStatusChanged,
			this
		);
		const disposable = new Disposable(() => {
			if (localActiveKernel.connection) {
				...
				localActiveKernel.connection.connection.statusChanged.disconnect(
					onStatusChanged,
					this
				);
			}
