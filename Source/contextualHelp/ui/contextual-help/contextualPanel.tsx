...
export class ContextualPanel extends React.Component<IContextualPanelProps> {
	...
	private mainPanelToolbarRef: React.RefObject<HTMLDivElement> =
		React.createRef();
	...
	public render() {
		...
		if (this.props.hideUI) {
			return (
				<div
					id="main-panel"
					className="native-editor-celltoolbar-middle">
					<div className="styleSetter">
						<style>{`${
							this.props.rootCss ? this.props.rootCss : ""
						}`}</style>
					</div>
		...
		const progressBar =
			this.props.busy || !this.props.loaded ? <Progress /> : undefined;
	...
	private renderHeader() {
		if (
			this.props.currentExecutionCount > 0 &&
			this.props.cellVMs &&
			this.props.cellVMs.length > 0
		) {
			if (
				...
				return (
					<div>
						<div className="cell-header">
							Code under cursor:{" "}
							{this.props.cellVMs[0].cell.data.source}
						</div>
						<hr />
					</div>
				);
			...
			return (
				<div>
					...
					<hr />
					<div className="cell-header">
						Kernel must be started in order to get results
					</div>
				</div>
			);
		}
