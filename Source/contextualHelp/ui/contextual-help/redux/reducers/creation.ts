...
export namespace Creation {
	...
	export function addAndFocusCell(
		arg: NativeEditorReducerArg<IAddCellAction>,
	): IMainState {
	...
	export function insertAboveAndFocusCell(
		arg: NativeEditorReducerArg<IAddCellAction & ICellAction>,
	): IMainState {
	...
	export function insertBelowAndFocusCell(
		arg: NativeEditorReducerArg<IAddCellAction & ICellAction>,
	): IMainState {
	...
	export function insertAboveFirstAndFocusCell(
		arg: NativeEditorReducerArg<IAddCellAction>,
	): IMainState {
	...
	export function insertExistingAbove(
	...
	export function insertNewAbove(
		arg: NativeEditorReducerArg<ICellAction & IAddCellAction>,
	): IMainState {
	...
	export function insertExistingBelow(
		arg: NativeEditorReducerArg<
			ICellAction & IAddCellAction & { cell: ICell }
		>,
	): IMainState {
		const newVM = prepareCellVM(arg.payload.data.cell);
	...
	export function insertAboveFirst(
		...
		const firstCellId =
			arg.prevState.cellVMs.length > 0
				? arg.prevState.cellVMs[0].cell.id
				: undefined;
	...
	export function addNewCell(
		...
		return insertBelow({
			...
			payload: {
				...arg.payload,
				data: {
					cellId: getSelectedAndFocusedInfo(arg.prevState)
						.selectedCellId,
					newCellId: arg.payload.data.newCellId,
				},
			},
	...
	export function finishCell(
		arg: NativeEditorReducerArg<IFinishCell>,
	): IMainState {
		return Helpers.updateOrAdd(
			{
				...arg,
				payload: { ...arg.payload, data: arg.payload.data.cell },
			},
			(c: ICell, s: IMainState) => prepareCellVM(c),
		);
	}

	export function deleteAllCells(
		arg: NativeEditorReducerArg<IAddCellAction>,
	): IMainState {
				...
				if (
					...
					if (nextOrPrev >= 0) {
						newVMs[nextOrPrev] = {
							...
							selected: true,
							focused:
								previousSelection.focusedCellId ===
								arg.payload.data.cellId,
						};
					}
				}
