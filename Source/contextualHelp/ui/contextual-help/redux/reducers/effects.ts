				...
				if (typeof removeFocusIndex === "number") {
					prevState = unfocusCell({
						...
						payload: {
							...arg.payload,
							data: {
								cellId: prevState.cellVMs[removeFocusIndex].cell
									.id,
							},
						},
					});
		...
		if (arg.payload.data.cellId !== selectionInfo.selectedCellId) {
			...
			if (removeFocusIndex >= 0) {
				prevState = unfocusCell({
					...
					payload: {
						...arg.payload,
						data: {
							cellId: prevState.cellVMs[removeFocusIndex].cell.id,
						},
					},
				});
				prevState = deselectCell({
			...
			if (
				...
				newVMs[addIndex] = {
					...newVMs[addIndex],
					focused:
						typeof shouldFocusCell === "boolean"
							? shouldFocusCell
							: someOtherCellWasFocusedAndSelected,
					selected: true,
					cursorPos: arg.payload.data.cursorPos,
				};
			}
