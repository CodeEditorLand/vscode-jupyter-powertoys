...
export class CellOutput extends React.Component<ICellOutputProps> {
	...
	// eslint-disable-next-line @typescript-eslint/no-explicit-any
	private static ansiToHtmlClass_ctor:
	...
	public shouldComponentUpdate(
		...
		if (
			nextProps.themeMatplotlibPlots !== this.props.themeMatplotlibPlots
		) {
			return true;
		}
		...
		if (
			nextProps.cellVM.cell.data.cell_type !==
			this.props.cellVM.cell.data.cell_type
		) {
			return true;
		}
		...
		if (
			nextProps.cellVM.cell.data.outputs !==
			this.props.cellVM.cell.data.outputs
		) {
			return true;
		}
		if (
			!this.isCodeCell() &&
			nextProps.cellVM.cell.data.source !==
				this.props.cellVM.cell.data.source
		) {
			return true;
		}
	...
	private renderCodeOutputs = () => {
		...
		if (
			...
			const trim = this.props.cellVM.cell.data.metadata.tags
				? this.props.cellVM.cell.data.metadata.tags[0]
				: "";
			// Render the outputs
			const outputs = this.renderOutputs(
				this.getCodeCell().outputs,
				trim
			);
			return outputs;
	...
	private computeOutputData(output: nbformat.IOutput): ICellOutputData {
		...
		if (input && input.hasOwnProperty("application/json")) {
			...
			try {
				if (ansiRegex.default().test(concatted)) {
					const converter = new CellOutput.ansiToHtmlClass(
						CellOutput.getAnsiToHtmlOptions()
					);
		...
		if (Array.isArray(data)) {
			data = concatMultilineString(
				data as nbformat.MultilineString,
				true
			);
		}
		...
		if (mimeType === "text/latex") {
			data = fixMarkdown(
				concatMultilineString(data as nbformat.MultilineString, true),
				true
			);
		}
		...
		try {
			...
			if (/^text\//.test(data.mimeType)) {
				...
				return {
					...
					extraButton,
					doubleClick: noop,
					outputSpanClassName: this.props.themeMatplotlibPlots
						? undefined
	...
	private renderOutput = (
		...
		transformedList.forEach((transformed, index) => {
			...
			if (mimeType && isMimeTypeSupported(mimeType)) {
				...
				let className = transformed.output.isText
					? "cell-output-text"
					: "cell-output-html";
				className = transformed.output.isError
					? `${className} cell-output-error`
					: className;
				...
				if (transformed.outputSpanClassName) {
					buffer.push(
						<div
							role="group"
							key={index}
							onDoubleClick={transformed.doubleClick}
							className={className}>
		...
		if (
			transformedList.some(
				(transformed) => transformed.output.renderWithScrollbars
			) &&
