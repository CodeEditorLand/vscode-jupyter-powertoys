...
function createTestLogger() {
	...
	log4js.configure({
		appenders: {
			reduxLogger: {
				type: "file",
				filename: "rchiodo-powertools-redux.log",
			},
		},
 ...
 */
const addMessageDirectionMiddleware: Redux.Middleware =
	(_store) => (next) => (action: Redux.AnyAction) => {
...
export function createStore<M>(
	...
	const middleware = createMiddleWare(
		...
		postOffice,
		transformLoad,
	).concat([addMessageDirectionMiddleware]);
