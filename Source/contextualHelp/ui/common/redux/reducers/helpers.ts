...
import { CellState, ICell, ICellViewModel, IMainState } from "../../types";
...
export namespace Helpers {
	...
	export function firstCodeCellAbove(
		state: IMainState,
	...
	export function updateOrAdd(
		...
		const newExecutionCount = arg.payload.data.data.execution_count
			? Math.max(
					...
					parseInt(
						arg.payload.data.data.execution_count.toString(),
						10,
					),
				)
			: arg.prevState.currentExecutionCount;
		...
		if (index >= 0) {
			...
			const finished =
				arg.payload.data.state === CellState.finished ||
				arg.payload.data.state === CellState.error;
