...
export class ContentPanel extends React.Component<IContentPanelProps> {
	...
	private containerRef: React.RefObject<HTMLDivElement> =
		React.createRef<HTMLDivElement>();
	private throttledScrollIntoView = throttle(
	...
	public componentWillReceiveProps(prevProps: IContentPanelProps) {
		...
		if (
			!fastDeepEqual(
				prevProps.cellVMs.map(this.outputCheckable),
				this.props.cellVMs.map(this.outputCheckable)
			)
		) {
			this.scrollToBottom();
		}
	...
	public computeIsAtBottom(parent: HTMLDivElement): boolean {
		if (this.bottomRef.current) {
			...
			return (
				this.bottomRef.current.offsetTop - parent.offsetTop - 2 <
	...
	public render() {
		const className = `${
			this.props.scrollBeyondLastLine
				? "content-panel-scrollBeyondLastLine"
				: ""
		}`;
		return (
			<div
				id="content-panel-div"
