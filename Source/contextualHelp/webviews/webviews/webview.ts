...
export abstract class Webview implements IWebview {
	...
	protected async generateLocalReactHtml() {
		...
		const uris = this.options.scripts.map((script) =>
			this.webviewHost!.webview!.asWebviewUri(Uri.file(script))
		);
		...
		testFiles
			.filter(
				(f) =>
					f.fsPath.toLowerCase().endsWith(".js") ||
					f.fsPath.toLowerCase().endsWith(".js.map")
			)
		...
		const forceTestMiddleware =
			process.env.VSC_JUPYTER_WEBVIEW_TEST_MIDDLEWARE || "false";
		return `<!doctype html>
        <html lang="en">
            <head>
                <meta charset="utf-8">
		...
		try {
			if (this.webviewHost?.webview) {
				const localFilesExist = await Promise.all(
					this.options.scripts.map((s) => localFileExists(s))
				);
				if (localFilesExist.every((exists) => exists === true)) {
					...
					this.webviewHost.webview.html =
						await this.generateLocalReactHtml();

					// Hook up class specific events after load
					this.postLoad(this.webviewHost);
				} else {
					// Indicate that we can't load the file path
					const badPanelString = `<html><body><h1>${this.options.scripts.join(
						", "
					)} is not a valid file name</h1></body></html>`;
