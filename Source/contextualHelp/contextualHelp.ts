	...
	implements vscode.Disposable, IStatusParticipant
{
	...
	constructor(
		...
		vscode.window.onDidChangeNotebookEditorSelection(
			this.activeNotebookSelectionChanged,
			this,
			disposables
		);
	}
		...
		try {
			...
			const result =
				kernel && code && code.length > 0
					? await kernel.connection.connection.requestInspect({
							code,
							cursor_pos,
							detail_level,
					  })
			...
			if (
				...
				"text/plain" in result.content.data
			) {
				...
				if (extension) {
					...
					if (exports && (exports as any).getKernelService) {
						...
						this.kernelService
							...
							?.connection.kernelSocket?.onDidChange(
