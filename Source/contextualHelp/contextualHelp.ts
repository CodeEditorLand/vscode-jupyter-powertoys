...
import {
	escapeStringToEmbedInPythonCode,
	execCodeInBackgroundThread,
} from "../common/backgroundExecution";
...
} from "./jupyter-extension/types";
import { MessageMapping, WindowMessages } from "./messages";
import { StatusProvider } from "./statusProvider";
import { IStatusParticipant, IWebviewViewProvider, Resource } from "./types";
...
export class ContextualHelp
	extends WebviewViewHost<MessageMapping>
	implements vscode.Disposable, IStatusParticipant
{
	...
	public showHelp(editor: vscode.TextEditor) {
		...
		if (
			...
			(this.lastHelpRequest?.cursor_pos === cursor_pos ||
				this.lastHelpRequest?.lineNumber === lineNumber) &&
	...
	protected postMessage<M extends MessageMapping, T extends keyof M>(
		type: T,
		payload?: M[T],
	): Promise<void> {
	...
	protected async inspect(
		...
		try {
			...
			const content = await this.doInspect(
				code,
				cursor_pos,
				document,
				token,
	...
	private async activeEditorChanged(
		...
		if (
			...
			isNotebookCell(vscode.window.activeTextEditor.document)
		) {
	...
	private async activeNotebookSelectionChanged(
		e: vscode.NotebookEditorSelectionChangeEvent,
	) {
